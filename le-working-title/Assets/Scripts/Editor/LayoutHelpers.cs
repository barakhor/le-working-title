...
namespace Editor
{
    ...
    public class TabsBlock
    {
        ...
        public           int                        CurrentMethodIndex = -1;
        ...
        public void Draw()
        {
            ...
            using(new VerticalBlock(EditorStyles.helpBox))
            {
                using(new HorizontalBlock())
                {
                    ...
                    {
                        GUIStyle buttonStyle = i == 0                 ? EditorStyles.miniButtonLeft :
                                               i == (keys.Length - 1) ? EditorStyles.miniButtonRight :
                                                                        EditorStyles.miniButtonMid;
                        using(new ColoredBlock(currentGuiMethod == methods[keys[i]] ? Color.grey : Color.white))
                            if(GUILayout.Button(keys[i], buttonStyle))
                                SetCurrentMethod(i);
                    }
                }

                GUILayout.Label(keys[CurrentMethodIndex], EditorStyles.centeredGreyMiniLabel);
                currentGuiMethod();
            }
        }
